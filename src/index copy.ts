...
const watchTelegram = async () => {
    ...
    if (sessionString) {
     ...
     // ✅ Verifica se está conectado antes de adicionar handler
    log("📡 Status conectado:", client.connected);
    client.addEventHandler(async (event: NewMessageEvent) => {
        const message = event.message;
        const chatId = message.chatId?.toString();
        console.log('CHAT ID::', chatId);
        const groupIds = ["-1003972273645"];
        if (!chatId || !groupIds.includes(chatId)) return;
        log("Mensagem do grupo:", message.text);

        const msg:propsBet = parseMensagem(message.text)
        log("Estrutura:", msg)
        if(BetInProgress) await new Promise(resolve => setTimeout(resolve, 10000))
        placeBet(msg)

    }, new NewMessage({}));
    // ✅ Isso segura o processo vivo indefinidamente
    await new Promise(() => {});
};
